...
import { SOBRE_SPENDING_CATS } from "@/lib/types";
import type { Categoria, Envelope, Moneda } from "@/lib/types";
...
export default async function NuevoSobrePage() {
  ...
  const [{ data: pItemsRaw }, { data: envsRaw }] = await Promise.all([
    supabase
      .from("budget_items")
      ...
      .eq("mes", mes)
      .eq("anio", anio)
      .in("categoria", [...SOBRE_SPENDING_CATS])
  ...
  const personalLines: LineOption[] = ((pItemsRaw ?? []) as LineRow[])
    .filter((i) => !usedPIds.has(i.id) && !usedPNames.has(`${i.categoria}|${i.concepto}`))
    .map((i) => ({
      id: i.id,
      concepto: i.concepto,
      monto: Number(i.monto),
      moneda: i.moneda,
      label: `${t(`categoria.${i.categoria}` as `categoria.${Categoria}`)} · ${i.concepto} — ${money(
        Number(i.monto),
        i.moneda,
      )}`,
    }));
